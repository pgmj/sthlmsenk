...
Frågorna har stor variation i svarskategorierna, vilket beskrivs i PDF-filen med frågor, och även framgår om man klickar på </CODE> vid denna sidas rubrik högst upp och tittar på källkoden till analysen där omkodning ("recode") av svarskategorierna finns med tidigt i koden (innan denna text).
...
#          Årskurs = ARSKURS) %>%
#   mutate(År = factor(ar)) %>% 
#   ggplot(aes(x = År, y = Ålder, group = Item, color = Item)) +
#   geom_point(size = 2) +
#   geom_line() +
#   scale_fill_viridis_d() +
...
Detta reducerar sampelstorleken från `r nrow(df.omit.na)` till `r nrow(df.deb2)` respondenter.
...
df.deb2 <- df.deb2 %>%
  mutate(across(everything(), ~ recode(.x, "1:11=0;12=1;13=2;14=3;15=4;16=5;17=6;18:20=7")))
...
- Fråga 37 (f36 i data), *Hur många gånger har du druckit så mycket alkohol att du känt dig berusad under den senaste 4-veckorsperioden?*
Fördelningen av svaren är märklig, se histogram nedan
...
Item F14 är inte formulerad enligt samma tydliga frekvensfråga som de två tidigare frågorna.
Den har sedan tidigare kodats enligt nedan:

recode(df$F14,"'Nej, jag har aldrig rökt'=0;
                 'Nej, bara provat hur det smakar'=1;
                 'Nej, jag har rökt men slutat'=2;
                 'Ja, ibland men inte varje dag'=3;
                 'Ja, dagligen'=4;
...
Och inför test av index kodas den
"0:2=0;3=1;4=2"
...
recode(df$FNY12020,"'Nej, jag har aldrig rökt e-cigaretter'=0;
                 'Nej, bara provat hur det smakar'=1;
                 'Nej, jag har rökt e-cigaretter men slutat'=2;
                 'Ja, ibland men inte varje dag'=3;
                 'Ja, dagligen'=4;
...
F18r = snus
f36r = alkohol
F49r = narkotika (cannabis eller annan narkotika)
GNY12020r = e-cigaretter
...
df %>%
  ...
  mutate(
    bruk4v = case_when(
      ...
      F14r > 1 & F36new > 1 ~ "Tobak och alkohol mer än en gång",
      F14r > 1 & F49new > 1 ~ "Tobak och narkotika mer än en gång",
  ...
  mutate(bruk4v = factor(bruk4v, levels = c("Alla tre substanser mer än en gång",
                                            ...
                                            "Tobak och narkotika mer än en gång",
                                            "Tobak och alkohol mer än en gång",
                                            "En substans en gång",
                                            "Ej använt",
                                            "Svar saknas")
  )) %>% 
  ...
  labs(
    title = paste0(plotFaktor, " användning senaste 4 veckorna"),
    subtitle = "Uppdelat på kön"
  ) +
  facet_grid(ARSKURS ~ Kön)
