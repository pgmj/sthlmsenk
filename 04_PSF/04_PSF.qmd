...
df.indexR %>% 
  ...
  summarise(PSFmean = mean(PSFindex, na.rm = T)) %>% 
  ggplot(aes(y = PSFmean, x = SkolSDO, fill = Kön, color = Kön)) +
  geom_chicklet(radius = grid::unit(1.5, 'mm'),
                position = "dodge") +
  geom_hline(aes(yintercept = mean(PSFmean)), linetype = 2, color = "black") +
  annotate("text", label = "Medelvärde Stockholm Stad", y = 57, x = 3, color = "darkgrey") +
  scale_fill_manual('Kön', values = RISEpalette1[c(1,5)], aesthetics = c("color","fill")) +
  scale_x_discrete('Stadsdel', guide = guide_axis(n.dodge = 2)) +
  theme_rise() +
  ylab("Snittvärde") +
...
df.index %>% 
  ...
  filter(!is.na(SkolSDO)) %>% 
  group_by(SkolSDO,Kön) %>% 
  summarise(PSFmean = mean(PSMscore100, na.rm = T)) %>% 
  ggplot(aes(y = PSFmean, x = SkolSDO, fill = Kön, color = Kön)) +
  geom_chicklet(radius = grid::unit(1.5, 'mm'),
                position = "dodge") +
  geom_hline(aes(yintercept = mean(PSFmean)), linetype = 2, color = "black") +
